...
function getEntitySources(
  ...
  const sources = documents.flatMap((document) => {
    ...
    try {
      ...
      return entities[collection]?.includes(entity.name)
        ? [document.name]
        : [];
    } catch {
      return [];
          ...
          {timeline.map((item) => {
            ...
            const content = (
              <div className="w-full rounded-2xl border border-slate-700 bg-slate-800 p-6 transition-all duration-300 hover:border-cyan-500 hover:shadow-lg hover:shadow-cyan-900/30">
                ...
                <p className="mt-3 leading-7 text-slate-300">
                  {item.description ||
                    "لا يتوفر وصف مختصر لهذا الكيان حتى الآن."}
                </p>
            ...
            return (
              <motion.div
                ...
                transition={{
                  duration: 0.35,
                  delay: Math.min(item.station * 0.04, 0.24),
                }}
              >
                <div
                ...
                {href ? (
                  <Link
                    href={href}
                    className="w-full focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300 focus-visible:ring-offset-4 focus-visible:ring-offset-slate-900"
                  >
                    {content}
                  </Link>
                ) : (
